...
export function useBookList() {
  ...
  const {
    books, meta, searchQuery, isSearching, isLoading,
    searchComplete, perPage, sortBy, sortOrder
  } = state;
  ...
  const fetchBooks = useCallback(
    async (page: number = 1, itemsPerPage: number = perPage, sortColumn: string = sortBy, sortDirection: string = sortOrder) => {
      ...
      try {
        const response = await fetch(`/api/v1/books?page=${page}&per_page=${validItemsPerPage}&sort_by=${sortColumn}&sort_order=${sortDirection}`);
  ...
  const searchBooks = useCallback(
    async (query: string, page: number = 1, itemsPerPage: number = perPage, sortColumn: string = sortBy, sortDirection: string = sortOrder) => {
      ...
      try {
        const response = await fetch(`/api/v1/books/search?q=${encodeURIComponent(query.trim())}&page=${page}&per_page=${validItemsPerPage}&sort_by=${sortColumn}&sort_order=${sortDirection}`);
        const data = await response.json();
